...
sun_alt_limit <- -18         # Maximum solar altitude to consider (e.g., -18 for astronomical night)
min_msas_filter <- 0         # Minimum MSAS value to include
msas_limit <- 21.3           # Reference MSAS value for visualization (e.g., 21.3 for dark skies)
...
# Convert time to POSIXct, define night_id, and apply filters
df <- df %>%
  mutate(
  ...
  filter(
    sun_alt <= sun_alt_limit,         # Filter for sun_alt based on variable
    year(time) == selected_year,      # Filter for the selected year
    month(time) == selected_month,    # Filter for the selected month
    msas > min_msas_filter            # Exclude values below the minimum MSAS
  )
...
ggplot(df_night_stats, aes(x = date)) +
  ...
  geom_hline(yintercept = msas_limit, linetype = "dashed", color = "red", size = 0.8) +  # Reference line for MSAS limit
  labs(
    title = paste("Min und Max MSAS pro Nacht in", month.abb[selected_month], selected_year),
    x = "Datum [Tag]",
    y = "MSAS [mag/arcsec²]",
    color = "MSAS Value"
  ) +
